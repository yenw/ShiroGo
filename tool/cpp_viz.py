...
def plot_network(symbol, title="plot", symbol_input=['data'], save_format='pdf', shape=None, node_attrs={},
                 hide_weights=True):
    ...
    for node in nodes:
        ...
        if op == "null":
            if looks_like_weight(node["name"]):
                ...
                if input_name not in symbol_input:
                    continue
            ...
            label = r"Convolution\n%s/%s, %s%s" % ("x".join(_str2tuple(node["attrs"]["kernel"])),
                                                 "x".join(_str2tuple(node["attrs"]["stride"]))
                                                 if "stride" in node["attrs"] else "1",
                                                 node["attrs"]["num_filter"],
                                                 ", C=%s"%(node["attrs"]["num_group"])
                                                 if node["attrs"]["num_group"] != "1" else "")
            attr["fillcolor"] = cm[1]
        elif op == "FullyConnected":
                ...
                if input_name not in hidden_nodes:
                    ...
                    if draw_shape:
                        if input_node["op"] != "null":
                            ...
                            attr["label"] = label
                        elif input_name in symbol_input:
                            key = input_name
                            shape = shape_dict[key][1:]
                            label = "x".join([str(x) for x in shape])
                        else:
...
def python_json(fn):
    with open(fn,'r') as f:
        ...
        symbol_json['nodes'] = nodes
        return json.dumps(symbol_json);

symbol_input = ['data', 'mcts_p', 'mcts_bv']
symbol = mx.symbol.load_json(python_json('res.txt'))
plot_network(symbol, symbol_input=symbol_input, shape={"data":(13, 2*8+1, 19, 19)}, node_attrs={"shape":'rect',"fixedsize":'false'}).view()
